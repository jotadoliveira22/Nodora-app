...
impl AppState {
    pub fn with_registry<T>(&self, f: impl FnOnce(&Registry) -> Result<T>) -> Result<T> {
        let guard = self.registry.lock().map_err(|_| NodoraError::Internal("lock registro".into()))?;
        f(&guard)
    }

    pub fn with_ws<T>(&self, f: impl FnOnce(&OpenWorkspace) -> Result<T>) -> Result<T> {
        let guard = self.workspace.lock().map_err(|_| NodoraError::Internal("lock workspace".into()))?;
    ...
    pub fn with_ws_mut<T>(&self, f: impl FnOnce(&mut OpenWorkspace) -> Result<T>) -> Result<T> {
        let mut guard = self.workspace.lock().map_err(|_| NodoraError::Internal("lock workspace".into()))?;
